...
bool _Detector::detect(const std::vector<cv::Mat> &imgs_, 
            std::vector<std::vector<Detection> > &vec_detections_)
{
    ...
    for (int f = 0; f < (int)imgs_.size(); f++) 
    {
        ...
        for (int b = 0; b < fcount; b++) 
        {
            ...
            cv::Mat pr_img = preprocess_img(img, INPUT_W, INPUT_H); // 长宽比不变居中，边缘125填充
        ...
        doInference(*_context, _stream, _buffers, data, prob, BATCH_SIZE);
        ...
        for (int b = 0; b < fcount; b++) 
        {
            ...
            for (size_t j = 0; j < res.size(); j++) 
            {
                ...
                Detection detection;
                detection.rect = r;
                detection.conf = res[j].conf;
                detection.class_id = (int)res[j].class_id;
                detections.push_back(detection);
            }
            vec_detections_.push_back(detections);
        }
